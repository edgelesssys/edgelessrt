...
void payload::apply_relocations(void start_main(int(...)))
{
    const auto base = static_cast<const uint8_t*>(get_base());
    assert(base);
    ...
    // get infos from dynamic section
    for (; dyn->d_tag != DT_NULL; ++dyn)
        switch (dyn->d_tag)
        {
            case DT_STRTAB:
        ...
        switch (type)
        {
            ...
            case R_X86_64_GLOB_DAT:
                if (strcmp(
                        ...
                        "__libc_start_main") == 0)
                    *(void**)(base + rel.r_offset) =
                        reinterpret_cast<void*>(start_main);
                break;
            default:
                throw logic_error(
                    "unsupported relocation type " + to_string(type));
        }
    }
    ...
    for (size_t i = 0; i < jmprel_size / sizeof *jmprel; ++i)
    {
        ...
        switch (type)
        {
            case R_X86_64_JUMP_SLOT:
            {
                const auto symbol =
                    strtab + symtab[ELF64_R_SYM(rel.r_info)].st_name;
                const auto addr = __dlsym(0, symbol);
                if (addr)
                    *(const void**)(base + rel.r_offset) = addr;
                else
                {
    ...
    if (failed)
        throw logic_error("failed to apply jump relocations");
}
